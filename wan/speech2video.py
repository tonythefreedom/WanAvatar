...
def load_sharded_safetensors(checkpoint_dir, device="cpu"):
    ...
    if os.path.exists(index_file):
        ...
        # Fallback: load all safetensors files in directory
        shard_files = sorted(glob.glob(os.path.join(checkpoint_dir, "diffusion_pytorch_model*.safetensors")))
        tensors = {}
...
class WanS2V:
    ...
    def __init__(
        ...
        if not dit_fsdp:
            # Manual loading to avoid safetensors device issues with accelerate
            import json
            config_file = os.path.join(checkpoint_dir, "config.json")
            with open(config_file, 'r') as f:
                model_config = json.load(f)
            # Remove non-model keys
            for key in ['__name__', '_class_name', '_diffusers_version']:
