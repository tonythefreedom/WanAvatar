...
def update_language(language):
    if language == "English":
        ...
        return {
            GPU_memory_mode: gr.Dropdown(label="GPU 메모리 모드", info="Normal: 25G VRAM 사용, model_cpu_offload: 13G VRAM 사용"),
            teacache_threshold: gr.Slider(label="TeaCache 임계값", info="권장값 0.1, 0으로 설정시 TeaCache 가속 비활성화"),
            num_skip_start_steps: gr.Slider(label="시작 스텝 건너뛰기", info="권장값 5"),
            ...
            height: gr.Slider(label="높이"),
            exchange_button: gr.Button("🔄 너비/높이 교환"),
            adjust_button: gr.Button("이미지 기준 크기 조정"),
            guidance_scale: gr.Slider(label="가이던스 스케일"),
            num_inference_steps: gr.Slider(label="샘플링 스텝 수 (권장 50)"),
            text_guide_scale: gr.Slider(label="텍스트 가이던스 스케일"),
            audio_guide_scale: gr.Slider(label="오디오 가이던스 스케일"),
            motion_frame: gr.Slider(label="모션 프레임"),
            fps: gr.Slider(label="프레임 레이트"),
            overlap_window_length: gr.Slider(label="오버랩 윈도우 길이"),
            seed_param: gr.Number(label="시드 (양의 정수, -1은 랜덤)"),
            info: gr.Textbox(label="상태 정보"),
            video_output: gr.Video(label="생성 결과"),
            seed_output: gr.Textbox(label="시드"),
            video_path: gr.Video(label="비디오 업로드"),
            extractor_button: gr.Button("🎬 추출 시작"),
            info2: gr.Textbox(label="상태 정보"),
            audio_output: gr.Audio(label="생성 결과"),
            audio_path3: gr.Audio(label="오디오 업로드"),
            separation_button: gr.Button("🎬 분리 시작"),
            info3: gr.Textbox(label="상태 정보"),
            audio_output3: gr.Audio(label="생성 결과")
        }
    else:
        return {
